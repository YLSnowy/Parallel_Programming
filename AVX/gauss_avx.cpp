...
void Part1_SIMD_LU(){
    for(int k = 0;k < n; k++){
        m2 = _mm256_loadu_ps((float*)&(A[k][k]));
        int j = k + 1;
        for(;j + 16 < n; j += 16){
            m1 = _mm256_loadu_ps((float*)&(A[k][j]));
            m1 = _mm256_div_ps(m1,m2);
            _mm256_storeu_ps((float*)&(A[k][j]),m1);
        }
        A[k][k] = 1.0;
        for(int i = k + 1;i < n; i++){
			for(j = k + 1;j < n;j++){
...
void Part2_SIMD_LU(){
    for(int k = 0;k < n; k++){
        ...
        A[k][k] = 1.0;
        for(int i = k + 1;i < n; i++){
            m3 = _mm256_loadu_ps((float*)&(A[i][k]));
            for(j = k + 1;j + 16 < n;j += 16){
                m2 = _mm256_loadu_ps((float*)&(A[k][j]));
                m1 = _mm256_loadu_ps((float*)&(A[i][j]));
                m2 = _mm256_mul_ps(m2,m3);
                m1 = _mm256_sub_ps(m1,m2);
                _mm256_storeu_ps((float*)&(A[i][j]),m1);
            }
            A[i][k] = 0.0;
        }
    }
    ...
    for(int k = 0;k < n; k++){
        ...
        for(int i = k + 1;i < n; i++){
            ...
            for(j = k + 1;j + 16 < n;j += 16){
	    		m2 = _mm256_loadu_ps((float*)&(A[k][j]));
	    		m1 = _mm256_loadu_ps((float*)&(A[i][j]));
	    		m2 = _mm256_mul_ps(m1,m2);
	    		m1 = _mm256_sub_ps(m1,m3);
            	_mm256_storeu_ps((float*)&(A[i][j]),m1);
            }
            A[i][k] = 0.0;
        }
...
void Align_SIMD_LU(){
    for(int k = 0;k < n; k++){
        ...
        for(int i = k + 1;i < n; i++){
			...
			j = k + 1;
			while((k * n + j) % 8 != 0) {
            	A[k][j] = A[i][j] - A[i][k] * A[k][j];
            	j++;
        	}
            for(;j + 16 < n;j += 16){
                m2 = _mm256_loadu_ps((float*)&(A[k][j]));
                m1 = _mm256_loadu_ps((float*)&(A[i][j]));
                m1 = _mm256_mul_ps(m1,m2);
                m1 = _mm256_sub_ps(m1,m3);
                _mm256_storeu_ps((float*)&(A[i][j]),m1);
            }
            A[i][k] = 0.0;
